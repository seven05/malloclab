...
int mm_init(void)
{
    ...
    PUT(heap_listp + (4*WSIZE), PACK(2*DSIZE,1)); // prologue footer 생성
    PUT(heap_listp + (5*WSIZE), PACK(0,1)); // epilogue block header 생성
    first_freep = heap_listp + (2*WSIZE);
...
static void remove_freeb(void *bp){
    if (bp == first_freep) {
        PREV_FREE_POINTER(NEXT_FREE_POINTER(bp)) = NULL;
        first_freep = NEXT_FREE_POINTER(bp);
    } else {
        NEXT_FREE_POINTER(PREV_FREE_POINTER(bp)) = NEXT_FREE_POINTER(bp);
        PREV_FREE_POINTER(NEXT_FREE_POINTER(bp)) = PREV_FREE_POINTER(bp);
    }
}

static void add_freeb(void *bp){
    // 합쳐진 블록의 next prev 수정
    NEXT_FREE_POINTER(bp) = first_freep; // 원래있던 루트의 next값을 나의 next값으로
    PREV_FREE_POINTER(bp) = NULL;   // 내 prev 값은 NULL 
    if (first_freep) {
        PREV_FREE_POINTER(first_freep) = bp;// 다음 가용리스트 prev값을 현재 bp를 가르키도록함
    }
    first_freep = bp;    // 루트의 next값을 현재bp로
}
...
static void *coalesce(void *bp){
    ...
    //case4 앞뒤 모두 가용상태
        ...
        if (PREV_BLKP(bp) != NULL && NEXT_BLKP(bp) != NULL) {
            // 다음블록 처리
            remove_freeb(NEXT_BLKP(bp));
            // 이전블록 처리
            remove_freeb(PREV_BLKP(bp));
...
static void place(void *bp, size_t asize)
{
    ...
    if ((csize-asize) > (2*DSIZE)){    // 할당크기를 뺀뒤에 16바이트 이상남으면 분할
        ...
        PUT(HDRP(bp), PACK(csize-asize,0)); // 나머지 블록을 가용블록으로 선언
